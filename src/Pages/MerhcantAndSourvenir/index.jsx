...
const MerhcantAndSourvenir = () => {
  ...
  const items = [
    { label: "MerhcantAndSourvenir", url: "/MerhcantAndSourvenir" },
  ];

  const handleGetProductsFromMerchant = ({ merchant }) => {
    let products = merchant.detail.products;
      ...
      return (
        <Card
          ...
          image={
            ...
            "/merchant-souvenir/products/" +
            product.thumbnail
          }
          title={product.nama}
          price={product.harga}
          merchant={merchant.name}
          address={merchant.lokasi.alamat}
          rating={1}
        />
      );
    });
    ...
    <PortalTemplate items={items}>
      <div className="relative h-72 bg-center bg-cover bg-hero-merchant -z-50 xl:h-[400px]">
        ...
        <div className="absolute px-4 py-2 font-semibold top-10 border-white border-2 rounded-2xl flex items-center text-white right-20 w-62 ml-5">
          <p className="text-sm">
            Kecamatan Ciemas, <br /> Aksesoris Geopark
          </p>
      ...
      <section className="z-50 px-2 -mt-10 md:px-10">
        <div className="flex flex-col px-4 py-10 bg-green-100 rounded-lg rounded-t-[3rem] text-primary lg:px-14">
          ...
          <div className="flex flex-col text-center space-y-4 mt-16 mb-10">
            <h1 className="text-xl md:text-3xl xl:text-5xl font-bold lg:w-[800px] xl:mx-auto">
              TENTUKAN BARANG YANG AKAN DIBERIKAN
            </h1>
            <p className="text-sm md:text-lg xl:text-xl">
              Temukan oleh-oleh yang akan diberikan kepada orang terkasih mu !
            </p>
          </div>
          ...
          <div className="flex flex-col">
            {/* Title */}
            <div>
              <p className="text-base md:text-lg xl:text-xl font-bold text-secondary">
                List Merchant & Souveniers
              </p>
              <p className="text-lg md:text-xl xl:text-3xl font-bold">
                Available Merchant & Souveniers
              </p>
            </div>
            ...
            <div className="flex items-center justify-between flex-nowrap w-full mb-5 mt-4 gap-2">
              ...
              <div className="relative">
                {/* Button */}
                <button
                  className="flex items-center px-4 py-2 bg-yellow-200 rounded-lg hover:bg-yellow-300"
                  onClick={toggleClass}
                >
                  <GiHamburgerMenu />
                ...
                <div
                  ...
                  } bg-green-50 absolute z-[100] w-72 left-0 shadow-2xl text-sm dark:text-gray-400 rounded-md py-5 px-5 md:w-[700px]`}
                >
                  {/* Header */}
                  <div className="flex justify-between mb-5">
                    <button className="flex items-center px-4 py-2 bg-yellow-200 hover:bg-yellow-300 rounded-lg">
                      <BiTrash></BiTrash>
                      <p className="ml-2 font-medium" onClick={toggleClear}>
                        Clear All
                      </p>
                    </button>
                    <button>
              ...
              <div className="flex items-center">
                <p className="mr-4 text-[10px] hidden md:block xl:text-lg">
          ...
          <div className="flex flex-wrap gap-5 justify-center lg:justify-between xl:gap-10 mb-10">
            {merchant.map((item, index) => {
              let products = item.detail.product;			  
              return products
                ? products.map((product) => {
                    return (
                      <Card
						...
						link={"/merchant-sourvenir/detail/" + product.kode_barang}
						rating={3}
                      />
                    );
                  })
          ...
          <div className="relative bg-center bg-cover bg-article-merchant h-96 rounded-md overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 flex flex-col items-center justify-center w-full h-full text-white md:items-start md:mx-10">
              ...
              <button className="flex items-center px-8 py-4 bg-yellow-300 rounded-md">
                <span className="font-semibold text-black">Lihat Artikel</span>
              </button>
            </div>
          </div>
        </div>
