							...
							if(index == 0){
								return(
									<div className="relative col-span-2 overflow-hidden rounded-1xl">
										...
										<img src={process.env.REACT_APP_API_BASE_URL+ "/storage/event/" + item.flyer_image} alt="event" className="h-full bg-center bg-cover filter brightness-50 rounded-2xl"/>
										<div className="absolute flex flex-col items-start justify-end w-full h-full bottom-6 sm:bottom-10 left-2 sm:left-4">
											<p className="text-xs md:text-sm text-white">Rp. {item.harga}</p>
											...
											<p className="flex items-center space-x-3 mb-2 text-xs md:text-sm text-white color-white ">
												...
												{item.start_event.date}
												<BiMap className='mr-1'/>
												{/* {item.wisata_id} */}
												{item.wisata?.lokasi?.alamat}
											</p>
											<div>
												<button 
